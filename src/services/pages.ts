  ...
  async fromModule(relativeDirPath: string): Promise<Page | null> {
    ...
    if (moduleIndexPath) {
      ...
      return {
        ...
        generateChildren: page.generateChildren,
      };
    }
  },
};
  ...
  private async reloadPages() {
    ...
    while (subpathsToCheck.length > 0) {
      ...
      if (indexPage) {
        ...
        if (indexPage.generateChildren) {
          const childPages = (await indexPage.generateChildren())
            .filter((childPage) => childPage.published)
            .map((childPage) => ({
              ...childPage,
              permalink: indexPage.permalink + childPage.permalink,
            }));
          console.log(`Dynamic child pages found for path ${subpath}`);
